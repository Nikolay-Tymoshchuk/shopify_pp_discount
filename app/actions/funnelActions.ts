...
import type { FunnelActionValidationErrors, FunnelExtendedByProducts } from '@/types/funnels.type';
...
export const funnelLoader = async ({
    ...
    try {
         ...
         * This is necessary to ensure that when creating or editing a Funnel,
         * these products do not appear in the target list.
         * This way we avoid situations where one product can be targeted in
         * several funnels at the same time
         */
...
export async function funnelAction({ request, params }: ActionFunctionArgs) {
    ...
    return redirect(newFunnel ? `/app/funnels/${newFunnel.id}` : '/app/funnels/new');
}
